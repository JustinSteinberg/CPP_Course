...
AbilityList::AbilityList(const AbilityList &source)
{
  ...
  //      new_node->ability.weapon = source.front->ability.weapon;
  //      new_node->ability.strength = source.front->ability.strength;
  //      new_node->next = 
}
...
AbilityList &AbilityList::operator=(const AbilityList &source)
{
	AbilityNode * temp;
        ...
        if(source.front == NULL){
            ...
            front->ability.weapon = source.front->ability.weapon;
            front->ability.strength = source.front->ability.strength;
            //front->next = source.front->next;
            AbilityNode *curr = front;
            AbilityNode *orig = source.front;
            cerr << "Creating New Nodes" << endl;
            while(orig->next != NULL){
                curr->next = new AbilityNode();
                curr->next->ability.weapon = orig->next->ability.weapon;
                curr->next->ability.strength = orig->next->ability.strength;
                curr->next->next = NULL;
                orig = orig->next;
                curr = curr->next;
                cerr << "looping through" << endl;
            }
        }  
        return *this; 
}
