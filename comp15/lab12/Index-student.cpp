...
void Index::query(const string &toQuery, ostream &out) const {
        ...
        /* Start TODO */
        if(entries.find(toQuery) != entries.end()){
            unsigned int length = entries.at(toQuery).size();
            for(unsigned int i = 0; i < length; i++){
               out << "Word: " << entries.at(toQuery).at(i).word << " LineNum: " << entries.at(toQuery).at(i).lineno << ": "<< entries.at(toQuery).at(i).sentence << endl;
            }
        }
        else{
        ...
        while (position = getPosition(infile), getline(infile, line)) {
                ...
                while (ss >> word) {
                    ...
                    if(entries.find(word) != entries.end()){
                        if(shouldAdd(entries.at(word), e) == true){
                        ...
                        vector<Entry> myVec;
                        myVec.push_back(e);
                        pair<string,vector<Entry>> MyHash (word,myVec);
                        entries.insert(MyHash);
                    } 
                        /* End TODO */                
                }
        }
